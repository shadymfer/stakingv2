...
function StakePoolHome() {
  ...
  return (
    ...
    >
      <Head>
        <title>{stakePoolMetadata?.displayName ?? 'Shadies Staking'}</title>
        <meta name="description" content="Where Shadows go on missions for $CRIM tokens." />
      ...
      <Header />
      <div
        className="relative z-0 mx-10 mt-2 flex flex-col gap-4"
        style={{
      ...
      >
        <HeroLarge />
        <div className="text-lg text-center text-white text-medium-4"
                style={{
                  ...stakePoolMetadata?.styles,
                  color:
                    stakePoolMetadata?.colors?.fontColor ??
